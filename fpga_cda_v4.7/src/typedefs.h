...
struct QuantizedTensor
{
  ...
  uint16_t s[SIZE / 64];  // scaling factors as fp16 bit patterns (one per group, GS=64)
};

template <int dim, int hidden_dim, int n_layers, int n_heads, int n_kv_heads, int vocab_size, int seq_len, int GS>
struct RunState
{
  // current wave of activations (host-only, not sent to FPGA — keep as float)
  float x[dim];
  float xb[dim];
  float xb2[dim];
  float hb[hidden_dim];
  ...
  float q[dim];
  float k[(dim * n_kv_heads) / n_heads];
  float v[(dim * n_kv_heads) / n_heads];
...
struct TransformerWeights
{
  ...
  uint16_t token_embedding_table[vocab_size * dim]; // dequantized embeddings (fp16 bit patterns)

  // weights for rmsnorms (fp16 bit patterns, matching kernel half layout)
  uint16_t rms_att_weight[n_layers * dim]; // (layer, dim) rmsnorm weights
  uint16_t rms_ffn_weight[n_layers * dim]; // (layer, dim)
  ...
  // final rmsnorm
  uint16_t rms_final_weight[dim]; // (dim,) fp16 bit patterns
