...
from cda.cuda_attention_gqa import cuda_hw_attention_gqa
...
print(f"Loading pre-built CUDA kernels...")
...
for N in N_LIST:
    ...
    try:
        ...
        Qr = (Qr @ rot_t).float()
        pk2 = torch.randint(0, 256, (H_kv, N, D // 4), device=device, dtype=torch.uint8)
        nk2 = torch.randn(H_kv, N, device=device, dtype=torch.float32).abs() * 10
        nv2 = torch.randn(H_kv, N, device=device, dtype=torch.float32).abs() * 3
        t_k2v2 = bench(lambda: cuda_hw_attention_gqa(Qr, pk2, nk2,
                                               pk2, nv2,
                                               cb2, cb2, rot, scale, N, GROUP,
                                               k_bits=2, topk=topk))
    ...
    try:
        ...
        nv2 = torch.randn(H_kv, N, device=device, dtype=torch.float32).abs() * 3
        t_k4v2 = bench(lambda: cuda_hw_attention_gqa(Qr, pk4, nk4,
                                               pv2, nv2,
                                               cb4, cb2, rot, scale, N, GROUP,
                                               k_bits=4, topk=topk))
