...
from core.core.cuda_attention_final import _get_final
...
for N in [1024, 4096, 16384, 32768, 65536]:
    ...
    # 1. K-Score
    t_kscore = cuda_time(lambda: mod.score4b(Qr, pk4, nk4, cb4, N, scale))

    # Get scores for subsequent stages
    scores = mod.score4b(Qr, pk4, nk4, cb4, N, scale)
    ...
    # 4. V-Output
    if topk > 0:
        t_vout = cuda_time(lambda: mod.vsparse2b(vals, idx, pv2, nv2, cb2, topk, N, D))
        output_rot = mod.vsparse2b(vals, idx, pv2, nv2, cb2, topk, N, D)
    else:
        t_vout = cuda_time(lambda: mod.vfull2b(attn, pv2, nv2, cb2, N, D))
        output_rot = mod.vfull2b(attn, pv2, nv2, cb2, N, D)

    # 5. Rotation (H transform)
    t_rot = cuda_time(lambda: output_rot @ rot)

    # Also measure FP16 FlashAttention for reference
    try:
