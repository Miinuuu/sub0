...
def eval_ppl(model, tokenizer, fn, max_ctx=4096, stride=64, desc=""):
    """Growing-context PPL (matches bench_ppl_scale.py protocol used for
    Tab.~ppl in main body). At each pos ∈ {stride, 2*stride, ..., max_ctx},
    context is ids[:pos] and we predict ids[pos:pos+1]."""
...
def main():
    print("Loading Llama-3.1-70B-Instruct (4-way device_map=auto)...")
    t0 = time.time()
    ...
    print("\n" + "=" * 70)
    print("Bit-width ablation — Llama-3.1-70B WikiText-2 (PPL, KV memory):")
