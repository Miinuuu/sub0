...
|-------------------------------|-------------------------------------------------------|--------------------------------------------------------------------------------------------|
| Table 2 (compute-path iso.)   | `results/table2_compute_path_iso/`                    | `bash reproduce.sh smoke` (1 cell) or `table2` (full sweep). Reports CDA, FA2 (FP16), R2 dequant+FA2, and R1 fused dequant+FA2 (Class (a) baselines) per (B,N) cell. |
...
| Fig 2 + Table A3 (throughput) | `results/tableA3_fig2_throughput/{4K,16K,64K,128K}/`  | `bash reproduce.sh fig2` (single cell) or full: `bench_capacity_sweep.py --batches 1 2 4 8 ...`. Sweeps FA2, CDA, plus the Class (a) `DEQUANT_FA2` (R2) and `FUSED_DEQUANT_FA2` (R1) baselines via `CDA_DECODE_BACKEND={dequant_fa2,fused_dequant_fa2}` (skip via `--skip-dequant-fa2` / `--skip-fused-dequant-fa2`). |
